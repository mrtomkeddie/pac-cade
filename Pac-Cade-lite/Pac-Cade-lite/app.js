...
function toGameView(theme) {
  ...
  qs('#game-title').textContent = state.selectedGame?.name || '';
  if (theme === 'arcade') qs('#arcade-marquee').textContent = state.selectedGame?.name || '';
  ...
  try {
    const el = qs('#app') || document.documentElement;
      ...
      if (window.ROMDATA && window.ROMNAME) {
        ...
        if (inner) {
          ...
          if (inner.contentDocument) {
            inner.contentDocument.location.replace(innerPage);
          ...
          try {
            ...
            if (w > 0 && h > 0) {
              ...
              if (!window.INNER_RATIO || !window.INNER_RATIO.w || !window.INNER_RATIO.h) {
...
async function startEmulatrixFromUrl(url, theme, innerPage) {
  ...
  try {
    ...
    qs('#game-title').textContent = 'Failed to load ROM (' + e.message + ')';
  }
}
...
function setupNavigation() {
  ...
  arcadeList.addEventListener('click', (e) => {
    const el = e.target.closest('.cabinet');
    if (!el) return;
  ...
  qs('#back-to-room').addEventListener('click', backToRoom);
  ...
  function updateFsUI() {
    ...
    qs('#app').classList.toggle('is-fullscreen', active);
    ...
    if (!active && state.currentView === 'game') { backToRoom(); }
  }
  ...
  qs('#fullscreen-btn').addEventListener('click', async () => {
    const el = qs('#app');
    if (!el) return;
    if (isFullscreen()) await exitFullscreen(); else await enterFullscreen(el);
    updateFsUI();
  });
  ...
  scaleSelect.addEventListener('change', (e) => setScaleMode(e.target.value));
  ...
  const GamepadBridge = (() => {
    ...
    function ensurePadOverlay() {
      let el = document.getElementById('pad-overlay');
      if (!el) {
        const style = document.createElement('style');
        style.textContent = `.pad-bar{position:fixed;right:12px;bottom:12px;background:rgba(15,20,34,.88);color:#e6e8ee;border:1px solid rgba(255,255,255,.18);border-radius:10px;padding:8px 10px;font:12px/1.2 system-ui;display:flex;gap:8px;z-index:9999}
        .pad-item{display:flex;align-items:center;gap:6px;opacity:.7}
        .pad-item.active{opacity:1}
        .pad-icon{min-width:24px;text-align:center;border:1px solid rgba(255,255,255,.25);border-radius:6px;padding:2px 6px;background:#121826}
        .pad-label{opacity:.9}`;
        document.head.appendChild(style);
        el = document.createElement('div');
        el.id = 'pad-overlay';
        el.className = 'pad-bar';
        document.body.appendChild(el);
      }
      const t = padType;
      const icons = t==='ps' ? {lp:'□', mp:'△', hp:'R1', lk:'×', mk:'○', hk:'R2', start:'Options'}
                  : t==='xbox' ? {lp:'X', mp:'Y', hp:'RB', lk:'A', mk:'B', hk:'RT', start:'Menu'}
                  : t==='switch' ? {lp:'Y', mp:'X', hp:'R', lk:'B', mk:'A', hk:'ZR', start:'+'}
                  : {lp:'X', mp:'Y', hp:'RB', lk:'A', mk:'B', hk:'RT', start:'Start'};
      const order = ['lp','mp','hp','lk','mk','hk','start'];
      el.innerHTML = order.map(k => `<div class="pad-item" data-k="${k}"><div class="pad-icon">${icons[k]}</div><div class="pad-label">${k.toUpperCase()}</div></div>`).join('');
    }
  ...
  function setBezel() {
    try {
      ...
      const app = qs('#app');
      app.classList.toggle('has-bezel', showBezel);
  ...
  async function setPacmanBezelImage() {
    try {
      ...
      if (res.ok) {
        ...
        qs('#app').classList.add('bezel-img');
      } else {
        clearBezelImage();
      }
    } catch {
  ...
  function clearBezelImage() {
    ...
    qs('#app').classList.remove('bezel-img');
  }

  qs('#rom-input').addEventListener('change', (e) => {
  ...
  function gameActionTemplate() {
    ...
    if (core === 'mame32' || core === 'mame2003') {
      if (base === 'mario') {
        return [
          ...
          { name: 'Jump', target: 'ControlLeft', default: 'Space' },
          { name: 'Start', target: 'Enter', default: 'Enter' },
      ...
      if (base === 'ssf2t') {
        return [
          ...
          { name: 'Left', target: 'ArrowLeft', default: 'KeyA' },
          { name: 'Right', target: 'ArrowRight', default: 'KeyD' },
          { name: 'LP', target: 'KeyQ', default: 'KeyU' },
          { name: 'MP', target: 'KeyW', default: 'KeyI' },
          { name: 'HP', target: 'KeyE', default: 'KeyO' },
          { name: 'LK', target: 'KeyA', default: 'KeyJ' },
          { name: 'MK', target: 'KeyS', default: 'KeyK' },
          { name: 'HK', target: 'KeyD', default: 'KeyL' },
          { name: 'Start', target: 'Enter', default: 'Enter' },
          { name: 'Coin', target: 'Digit1', default: 'Digit1' },
          { name: 'Service', target: 'F2', default: 'KeyP' },
        ];
      ...
      if (base === 'simpsons2p') {
        return [
          ...
          { name: 'Left', target: 'ArrowLeft', default: 'KeyA' },
          { name: 'Right', target: 'ArrowRight', default: 'KeyD' },
          { name: 'Hit', target: 'KeyS', default: 'KeyJ' },
          { name: 'Jump', target: 'KeyA', default: 'Space' },
          { name: 'Start', target: 'Enter', default: 'Enter' },
          { name: 'Coin', target: 'Digit1', default: 'Digit1' },
          { name: 'Service', target: 'F2', default: 'KeyP' },
        ];
      ...
      if (base === 'invaders') {
        return [
          ...
          { name: 'Left', target: 'ArrowLeft', default: 'KeyA' },
          { name: 'Right', target: 'ArrowRight', default: 'KeyD' },
          { name: 'Fire', target: 'ControlLeft', default: 'Space' },
          { name: 'Start', target: 'Digit1', default: 'Enter' },
    ...
    return [
      ...
      { name: 'Start', target: 'KeyS', default: 'Enter' },
      { name: 'Select', target: 'KeyA', default: 'ShiftRight' },
    ];
  }
...
async function loadGames() {
  try {
    ...
    state.games.arcade = Array.isArray(json.arcade) ? json.arcade : [];
    state.games.console = Array.isArray(json.console) ? json.console : [];
  } catch (e) {
    state.games.arcade = [];
    state.games.console = [];
  }
...
function cabinetHTML(g) {
  return `
    <button class="cabinet" data-core="${g.core}" data-name="${escapeHtml(g.name)}" data-rom="${escapeHtml(g.rom)}" data-aspect="${escapeHtml(g.aspect || '')}">
      <div class="cabinet-body">
        <div class="marquee">${escapeHtml(g.name)}</div>
        <div class="screen placeholder">Select to play</div>
...
function gameBoxHTML(g) {
  return `
    <button class="game-box" data-core="${g.core}" data-name="${escapeHtml(g.name)}" data-rom="${escapeHtml(g.rom)}" data-aspect="${escapeHtml(g.aspect || '')}">${escapeHtml(g.name)}</button>
  `;
}
